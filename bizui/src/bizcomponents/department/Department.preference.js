...
const internalImageListOf = (department) =>{
...
const internalSettingListOf = (department) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = DepartmentService[candidateServiceName] 
...
const executeTrans = (department,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_department/doJob",payload: payload})
...
const buildTransferModal = (department,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(department,targetComponent)}
...
const internalRenderExtraHeader = (department) =>{
	return null
}
const internalRenderExtraFooter = (department) =>{
	return null
}
...
class DepartmentPreference extends Component {
...
}))(Form.create()(DepartmentPreference))
