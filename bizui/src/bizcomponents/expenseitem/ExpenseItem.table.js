...
import styles from './ExpenseItem.table.less'
...
class ExpenseItemTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ExpenseItemBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/expenseItem/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
