...
const internalImageListOf = doctor => {
  ...
  const imageList = [{ title: '拍摄的图像', imageLocation: doctor.shotImage }];
...
const internalSettingListOf = doctor => {
  const userContext = null;
  const optionList = [];
...
const handleTransferSearch = (targetComponent, filterKey, newRequest) => {
  ...
  const pageNo = 1;
  const candidateReferenceService = DoctorService[candidateServiceName];
...
const executeTrans = (doctor, targetComponent) => {
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({ type: '_doctor/doJob', payload: payload });

    targetComponent.setState({ transferModalVisiable: false });
  });
};

const buildTransferModal = (doctor, targetComponent) => {
  const {
  ...
  return (
    <Modal
      ...
      onOk={() => executeTrans(doctor, targetComponent)}
...
const internalRenderExtraHeader = doctor => {
  return null;
};
const internalRenderExtraFooter = doctor => {
  return null;
};
...
class DoctorPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '医生',
      cardsFor: 'doctor',
      cardsSource: this.props.doctor,
      subItems: [
        {
          name: 'doctorAssignmentList',
          displayName: '医生的任务',
          type: 'doctorAssignment',
          count: doctorAssignmentCount,
          addFunction: true,
