...
import styles from './Doctor.updateform.less'
...
class DoctorUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, doctorId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextDoctorUpdateRow`,
        payload: {
          id: owner.id,
          type: 'doctor',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}医生: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
          ...
          <Form >
            <Row gutter={16}>
              ...
              <Col lg={6} md={12} sm={24}>
                <ImageComponent
                  buttonTitle={window.trans('doctor')}
