...
</c:if><c:if test="${param.referName ne 'period'}">
	<td class="select_candidate_td"
			data-candidate-method="./doctorScheduleManager/requestCandidatePeriod/${ownerBeanName}/${item.id}/"
			data-switch-method="./doctorScheduleManager/transferToAnotherPeriod/${item.id}/"
			data-link-template="./periodManager/view/${'$'}{ID}/">
		<span class="display_span">
			<c:if test="${not empty  item.period}">
			<a href='./periodManager/view/${item.period.id}/'>${item.period.displayName}</a>
			</c:if>
			<c:if test="${empty  item.period}">
			<a href='#'></a>
			</c:if>
			<button class="btn btn-link candidate-action">...</button>
		</span>
