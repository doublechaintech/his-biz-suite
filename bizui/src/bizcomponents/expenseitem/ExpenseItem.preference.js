...
const internalImageListOf = expenseItem => {
...
const internalSettingListOf = expenseItem => {
  const userContext = null;
  const optionList = [];
...
const handleTransferSearch = (targetComponent, filterKey, newRequest) => {
  ...
  const pageNo = 1;
  const candidateReferenceService = ExpenseItemService[candidateServiceName];
...
const executeTrans = (expenseItem, targetComponent) => {
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id = expenseItem.id;
    const serviceNameToCall = transferServiceName;

    const payload = { parameters, id, serviceNameToCall };

    //targetComponent.setState({transferModalVisiable:false})
    dispatch({ type: '_expenseItem/doJob', payload: payload });

    targetComponent.setState({ transferModalVisiable: false });
  });
};

const buildTransferModal = (expenseItem, targetComponent) => {
  const {
  ...
  return (
    <Modal
      ...
      onOk={() => executeTrans(expenseItem, targetComponent)}
...
const internalRenderExtraHeader = expenseItem => {
  return null;
};
const internalRenderExtraFooter = expenseItem => {
  return null;
};
    ...
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="费用类型">
        {expenseItem.expenseType == null
          ...
          : expenseItem.expenseType.displayName}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              expenseItem.expenseType ? expenseItem.expenseType.id : ''
            )
          }
...
class ExpenseItemPreference extends Component {
  state = {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '费用项目',
      cardsFor: 'expenseItem',
      cardsSource: this.props.expenseItem,
      subItems: [],
    };
...
}))(Form.create()(ExpenseItemPreference));
