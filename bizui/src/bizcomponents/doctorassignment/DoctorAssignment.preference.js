...
const internalImageListOf = doctorAssignment => {
...
const internalSettingListOf = doctorAssignment => {
  const userContext = null;
  const optionList = [];
...
const handleTransferSearch = (targetComponent, filterKey, newRequest) => {
  ...
  const pageNo = 1;
  const candidateReferenceService = DoctorAssignmentService[candidateServiceName];
...
const executeTrans = (doctorAssignment, targetComponent) => {
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id = doctorAssignment.id;
    const serviceNameToCall = transferServiceName;

    const payload = { parameters, id, serviceNameToCall };

    //targetComponent.setState({transferModalVisiable:false})
    dispatch({ type: '_doctorAssignment/doJob', payload: payload });

    targetComponent.setState({ transferModalVisiable: false });
  });
};

const buildTransferModal = (doctorAssignment, targetComponent) => {
  const {
  ...
  return (
    <Modal
      ...
      onOk={() => executeTrans(doctorAssignment, targetComponent)}
...
const internalSummaryOf = (doctorAssignment, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="医生">
        {doctorAssignment.doctor == null
          ? appLocaleName(userContext, 'NotAssigned')
          : doctorAssignment.doctor.displayName}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              'anotherDoctorId',
              doctorAssignment.doctor ? doctorAssignment.doctor.id : ''
            )
          }
      ...
      <Description term="部门">
        {doctorAssignment.department == null
          ? appLocaleName(userContext, 'NotAssigned')
          : doctorAssignment.department.displayName}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              'anotherDepartmentId',
              doctorAssignment.department ? doctorAssignment.department.id : ''
            )
          }
          style={{ fontSize: 20, color: 'red' }}
        />
      ...
      <Description term="更新时间">
        {moment(doctorAssignment.updateTime).format('YYYY-MM-DD')}
      </Description>
...
const renderPermissionSetting = doctorAssignment => {
  ...
  return (
    <PermissionSetting targetObject={doctorAssignment} targetObjectMeta={DoctorAssignmentBase} />
  );
};

class DoctorAssignmentPreference extends Component {
  state = {
...
}))(Form.create()(DoctorAssignmentPreference));
