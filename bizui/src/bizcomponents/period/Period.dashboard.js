...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/period/${targetComponent.props.period.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/period/${targetComponent.props.period.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
<Description term="序号" style={{wordBreak: 'break-all'}}>{period.id}</Description> 
<Description term="名称" style={{wordBreak: 'break-all'}}>{period.name}</Description> 
<Description term="代码" style={{wordBreak: 'break-all'}}>{period.code}</Description> 
