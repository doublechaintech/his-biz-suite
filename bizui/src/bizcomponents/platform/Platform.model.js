...
export default {
  ...
  effects: {
    ...
    *addDoctor({ payload }, { call, put }) {
      ...
      const data = yield call(PlatformService.addDoctor, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/platform/${id}/list/\DoctorList/Doctor+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateDoctor({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.updateDoctor, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/platform/${id}/list/\DoctorList/Doctor列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeDoctorList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.removeDoctorList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *addRegistration({ payload }, { call, put }) {
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.addRegistration, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/platform/${id}/list/\RegistrationList/Registration+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateRegistration({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.updateRegistration, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/platform/${id}/list/\RegistrationList/Registration列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeRegistrationList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.removeRegistrationList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
