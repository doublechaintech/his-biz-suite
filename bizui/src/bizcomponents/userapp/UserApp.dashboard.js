...
const imageList = userApp => {
  return [];
};

const internalImageListOf = userApp => defaultImageListOf(userApp, imageList);

const optionList = userApp => {
  return [{ title: '完全访问', value: userApp.fullAccess, parameterName: 'fullAccess' }];
};
...
const internalSummaryOf = (userApp, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="安全用户">
        {userApp.secUser == null
          ? appLocaleName(userContext, 'NotAssigned')
          : `${userApp.secUser.displayName}(${userApp.secUser.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '安全用户',
              'secUser',
              UserAppService.requestCandidateSecUser,
              UserAppService.transferToAnotherSecUser,
              'anotherSecUserId',
              userApp.secUser ? userApp.secUser.id : ''
            )
          }
...
class UserAppDashboard extends Component {
  ...
  render() {
    ...
    const {
      id,
      displayName,
      listAccessListMetaInfo,
      objectAccessListMetaInfo,
      listAccessCount,
      objectAccessCount,
    } = this.props.userApp;
      ...
      subItems: [
        {
          ...
          displayName: '访问列表',
          type: 'listAccess',
          count: listAccessCount,
          addFunction: true,
          role: 'listAccess',
          metaInfo: listAccessListMetaInfo,
          renderItem: GlobalComponents.ListAccessBase.renderItemOfList,
        },
        {
          ...
          addFunction: true,
          role: 'objectAccess',
          metaInfo: objectAccessListMetaInfo,
          renderItem: GlobalComponents.ObjectAccessBase.renderItemOfList,
        },
      ],
    };
