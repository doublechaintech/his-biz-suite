...
import styles from './Department.table.less'
...
class DepartmentTable extends PureComponent {
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = DepartmentBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/department/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
