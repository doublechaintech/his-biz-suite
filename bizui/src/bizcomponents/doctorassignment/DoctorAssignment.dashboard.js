...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/doctorAssignment/${targetComponent.props.doctorAssignment.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/doctorAssignment/${targetComponent.props.doctorAssignment.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
<Description term="序号" style={{wordBreak: 'break-all'}}>{doctorAssignment.id}</Description> 
<Description term="名称" style={{wordBreak: 'break-all'}}>{doctorAssignment.name}</Description> 
...
class DoctorAssignmentDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('doctor_assignment'),cardsFor: "doctorAssignment",
