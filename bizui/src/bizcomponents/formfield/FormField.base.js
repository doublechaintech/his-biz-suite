...
const displayColumns = [
  ...
  { title: fieldLabels.required, debugtype: 'bool', dataIndex: 'required', width: '9',render: (text, record)=>renderTextCell(text,record)},
  { title: fieldLabels.disabled, debugtype: 'bool', dataIndex: 'disabled', width: '9',render: (text, record)=>renderTextCell(text,record)},
  { title: fieldLabels.customRendering, debugtype: 'bool', dataIndex: 'customRendering', width: '9',render: (text, record)=>renderTextCell(text,record)},
  ...
  return (
	...
	<Col span={20}>
      ...
      <DescriptionList  key={formField.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.required} style={{wordBreak: 'break-all'}}>{formField.required}</Description> 
        <Description term={fieldLabels.disabled} style={{wordBreak: 'break-all'}}>{formField.disabled}</Description> 
        <Description term={fieldLabels.customRendering} style={{wordBreak: 'break-all'}}>{formField.customRendering}</Description> 
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {label, localeKey, parameterName, type, placeholder, defaultValue, description, fieldGroup, minimumValue, maximumValue, required, disabled, customRendering, candidateValues, suggestValues, formId} = formValuesToPack
	const form = {id: formId, version: 2^31}
	const data = {label, localeKey, parameterName, type, placeholder, defaultValue, description, fieldGroup, minimumValue, maximumValue, required, disabled, customRendering, candidateValues, suggestValues, form}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {label, localeKey, parameterName, type, placeholder, defaultValue, description, fieldGroup, minimumValue, maximumValue, required, disabled, customRendering, candidateValues, suggestValues, form} = objectToUnpack
	const formId = form ? form.id : null
	const data = {label, localeKey, parameterName, type, placeholder, defaultValue, description, fieldGroup, minimumValue, maximumValue, required, disabled, customRendering, candidateValues, suggestValues, formId}
	return data
}
