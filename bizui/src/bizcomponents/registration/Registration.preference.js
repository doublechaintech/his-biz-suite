...
import styles from './Registration.preference.less'
...
const internalImageListOf = (registration) =>{
...
const internalSettingListOf = (registration) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RegistrationService[candidateServiceName] 
...
const executeTrans = (registration,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_registration/doJob",payload: payload})
...
const buildTransferModal = (registration,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(registration,targetComponent)}
...
const internalRenderExtraHeader = (registration) =>{
	return null
}
const internalRenderExtraFooter = (registration) =>{
	return null
}
...
<Description term="Patient">{registration.patient==null?appLocaleName(userContext,"NotAssigned"):registration.patient.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Patient","profile","requestCandidatePatient",
	      "transferToAnotherPatient","anotherPatientId",registration.patient?registration.patient.id:"")} 
...
<Description term="Register">{registration.register==null?appLocaleName(userContext,"NotAssigned"):registration.register.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Register","profile","requestCandidateRegister",
	      "transferToAnotherRegister","anotherRegisterId",registration.register?registration.register.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class RegistrationPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Registration",cardsFor: "registration",cardsSource: this.props.registration,
  		subItems: [
...
}))(Form.create()(RegistrationPreference))
