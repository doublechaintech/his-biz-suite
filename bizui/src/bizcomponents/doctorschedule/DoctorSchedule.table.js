...
class DoctorScheduleTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/doctorSchedule/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
